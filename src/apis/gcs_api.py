...
class GcsAPI:
    ...
    def upload_ai_hero_images(
        self,
        pin_results: list[dict],
        generated_dir: Path,
    ) -> dict[str, str]:
        """
        Upload AI hero images to GCS for side-by-side review in Content Queue.

        Looks for the _ai_hero_image_path field in each pin result and uploads
        the raw AI hero image (not the rendered pin template).

        Args:
            pin_results: List of pin dicts with _ai_hero_image_path fields.
            generated_dir: Directory containing generated pin images.

        Returns:
            dict: pin_id -> public URL mapping for AI hero images.
        """
        if not self.client:
            return {}
        ...
        for pin in pin_results:
            pin_id = pin.get("pin_id", "")
            ai_path_str = pin.get("_ai_hero_image_path")

            if not pin_id or not ai_path_str:
                skipped += 1
                continue

            ai_path = Path(ai_path_str)
            if not ai_path.exists():
                # Try relative to generated_dir
                ai_path = generated_dir / ai_path.name
                if not ai_path.exists():
                    logger.debug("AI hero image not found for %s: %s", pin_id, ai_path_str)
                    skipped += 1
                    continue

            remote_name = f"ai-heroes/{pin_id}-ai-hero{ai_path.suffix}"
            url = self.upload_image(ai_path, remote_name)
            if url:
                url_map[pin_id] = url
                uploaded += 1
            else:
        ...
        logger.info(
            "GCS AI hero upload complete: %d uploaded, %d skipped out of %d pins",
            uploaded, skipped, len(pin_results),
        )
        return url_map
